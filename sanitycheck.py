...
def run_sanity_check(test_dir):
    # assert path.isdir(test_dir), FAIL_COLOR+f"No direcotry named {test_dir} found in {os.getcwd()}" # noqa
    print(
        "This script will perform a sanity test to ensure your code meets the criteria in the rubric.\n" # noqa
    )
    print(
        "Please enter the path to the file that contains your test cases for the GET() and POST() methods" # noqa
    )
    ...
    if not test_functions_for_post:
        ...
        if len(test_functions_for_post) == 1:
            ...
            print(
                FAIL_COLOR
                + "Please make sure you have two test cases for the POST() method." # noqa
                + "\nOne test case for EACH of the possible inferences (results/outputs) of the ML model.\n" # noqa
            )
            SANITY_TEST_PASSING = False
        ...
        if (
            ...
            print(
                FAIL_COLOR
                + "You do not seem to have TWO separate test cases, one for each possible prediction that your model can make." # noqa
            )
    ...
    print(
        WARN_COLOR
        + "This is a heuristic based sanity testing and cannot guarantee the correctness of your code." # noqa
    )
    print(
        WARN_COLOR
        + "You should still check your work against the rubric to ensure you meet the criteria." # noqa
    )
